  ...
  <div class="container mt-5 mb-5">
        ...
        <div id="registerForm" style="display:none;">
          <form id="registerFormData" method="POST" class="row g-4 mt-4">
            ...
            <div class="col-md-6">
              ...
              <input type="text" class="form-control" name="name" value="<?= htmlspecialchars($_SESSION['name'] ?? '') ?>" readonly>
            </div>

            <div class="col-md-6">
              <label class="form-label">นามสกุล <span>*</span></label>
              <input type="text" class="form-control" name="lastname" value="<?= htmlspecialchars($_SESSION['lastname'] ?? '') ?>" readonly>
            </div>

            <div class="col-md-6">
              <label class="form-label">รหัสพนักงาน</label>
              <input type="text" class="form-control" name="employee_id" value="<?= htmlspecialchars($_SESSION['employee_id'] ?? '') ?>" readonly>
            </div>

          <div class="col-md-6"> <label class="form-label">ตำแหน่งงาน</label> 
          <select name="position" class="form-select"> 
            ...
            <div class="col-md-6">
              ...
              <input type="text" name="department" class="form-control" value="<?= htmlspecialchars($_SESSION['department'] ?? '') ?>" readonly>
            </div>
            ...
            <div class="col-12 text-end mt-4">
              <button type="button" id="btnSaveRegister" class="btn btn-primary" disabled>บันทึกข้อมูล</button>
            </div>
    ...
    <script>
      document.getElementById('btnSearchTemplate').addEventListener('click', function() {
        ...
        if (keyword === '') {
          swal("ข้อมูลไม่ครบ", "กรุณากรอกชื่อหรือรหัสพนักงาน", {
            icon: "warning",
            buttons: {
              confirm: {
        ...
        fetch('search_usertemplate.php?q=' + encodeURIComponent(keyword))
          ...
          .then(data => {
            if (data && data.name) {
              ...
              document.getElementById('btnSaveRegister').disabled = false; // เปิดปุ่ม
            } else {
              swal("ไม่พบข้อมูล", "กรุณาตรวจสอบคำค้นหาอีกครั้ง", {
                icon: "warning",
                buttons: {
                  confirm: {
                    className: "btn btn-warning",
              ...
              document.getElementById('btnSaveRegister').disabled = true; // ปิดปุ่ม
            }
          })
